...
/*   Updated: 2022/12/22 01:05:07 by dfurneau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	hit_detect(t_gamedata *gd)
{
	...
	if (gd->pd->det->hit.xd > 0 && gd->pd->det->hit.yd > 0)
	{
		...
		c_log(gd, ft_strnjoin(6, "WALL SEG: ", tmp[2], " x: ", tmp[0],
				" y: ", tmp[1]), rgb_to_int(255, 255, 102), false);
		free(tmp[0]);
		free(tmp[1]);
		free(tmp[2]);
	}
	if (gd->pd->ddet->hit.xd > 0 && gd->pd->ddet->hit.yd > 0)
	{
		tmp[0] = ft_dtoa(gd->pd->ddet->hit.xd, 2);
		tmp[1] = ft_dtoa(gd->pd->ddet->hit.yd, 2);
		tmp[2] = ft_itoa(gd->pd->ddet->hit_seg);
		c_log(gd, ft_strnjoin(6, "DOOR SEG: ", tmp[2], " x: ", tmp[0],
				" y: ", tmp[1]), rgb_to_int(255, 153, 51), false);
		free(tmp[0]);
		free(tmp[1]);
		free(tmp[2]);
	}
...
void	debug(t_gamedata *gd)
{
	...
	if (DEBUG)
	{
		...
		if (MOUSE)
			mouse_pos(gd);
		c_log(gd, ft_strnjoin(3, "MEM: ", memory_usage(), " M"), -1, false);
		c_log(gd, ft_strnjoin(2, "FPS: ", tmp[4]), -1, false);
		while (i < 5)
			free(tmp[i++]);
		p_log(gd);
	}
