...
/*   Updated: 2022/12/19 00:55:06 by dfurneau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	plot_line(t_gamedata *gd)
{
	...
	{
		if (check_wall(gd, gd->m_ray.plot) == Door)
			if (check_door_pos(gd, gd->m_ray.plot) == true)
				break ;
...
void	minimap_rays(t_gamedata *gd)
{
	...
	{
		plot_line(gd);
		gd->m_ray.theta += ((FOV / WIDTH) * M_PI / 180.0);
